...
COVID19Final <- COVID19 %>%
  ...
  mutate(daily_change_average = rollapply(daily_change,7,mean, na.rm=T, align='right', fill=NA)) %>%
  mutate(daily_change_average = ifelse(!is.finite(daily_change_average),NA,round(daily_change_average,2))) %>%
  mutate(most_recent_change = daily_change_average[date == most_recent_date]) %>%
  ...
  mutate(textx = 0.2*(most_recent_date - first_death) + first_death) %>%
  ...
  COVID19Final$state_order2 <- factor(COVID19Final$state, levels = levels(reorder(COVID19Final$state, -COVID19Final$most_recent_change)))
```

*Plotting a moving average of new deaths per day for all 50 states, DC, and a national total. The number and color refer to magnitude of deaths for the day on which data is most recently available (`r data_day`).*
...
ggplotly(
          ...
          labs(title = "Deaths per Day - Sorted by State") +
